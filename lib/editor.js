...
class Editor {
  ...
  bindTestButton() {
    document.querySelector("#test").onclick = () => this.sendTestToBackend();
  }

  sendTestToBackend() {
    fetch("/mySQL", {
      method: "POST",
      body: JSON.stringify({ text: this.mde.value() }),
      headers: { 'content-type': 'application/json' }
    }).then(this.testResultReceived);
  }

  testResultReceived(res) {
    res.json().then(b => {
      console.log('Response: ', b)
      const testText = document.querySelector('#testResultText');
      if (b['result'] === true) {
        testText.innerHTML = 'Tests Pass!';
      } else {
        testText.innerHTML = 'Tests Fail';
      }
    });
  }
